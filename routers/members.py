...
@router.post("/members")
def create_member_endpoint(member_data , db=Depends(get_db), current_user: User = Depends(get_current_user)):
    if not current_user:
...
def update_member_endpoint(member_id : int , new_data : dict , db=Depends(get_db), current_user: User = Depends(get_current_user)):
    if not current_user:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Unauthorized")
